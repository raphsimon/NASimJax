...
# file src/jaxued/level_sampler.py.
#
# Notable modifications:
#   - Removed initial=0 argument to jax.nn.softmax, as it was no longer compatible with the jax version we're using.
#   - Applied pull request #6: fix: -jnp.inf scores could be set when updating the PLR buffer
...
from nasimjax.envs.network_generator import Level
    ...
    def update(
        ...
        def _replace():
            ...
            if level_extra is not None:
                new_sampler["levels_extra"] = jax.tree.map(
