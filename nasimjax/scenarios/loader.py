...
import nasimjax.scenarios.utils as u
from nasimjax.scenarios.scenario import Scenario
from nasimjax.scenarios.host import Host
...
ACCESS_LEVEL_MAP = {"user": u.USER_ACCESS, "root": u.ROOT_ACCESS}
...
class ScenarioLoader:
    ...
    def _validate_sensitive_hosts(self, sensitive_hosts):
        ...
        assert len(sensitive_hosts) > 0, (
            f"Number of sensitive hosts must be >= 1: {len(sensitive_hosts)} not >= 1"
        )
    ...
    def _validate_single_exploit(self, e_name, e):
        ...
        assert e[u.EXPLOIT_SERVICE] in self.services, (
            f"{e_name}. Exploit target service invalid: '{e[u.EXPLOIT_SERVICE]}'"
        )
    ...
    def _validate_single_privesc(self, pe_name, pe):
        ...
        assert pe[u.PRIVESC_PROCESS] in self.processes, (
            f"{pe_name}. {s_name} target process invalid: '{pe[u.PRIVESC_PROCESS]}'"
        )
    ...
    def _validate_host_config(self, addr, cfg):
        ...
        assert len(host_services) == len(set(host_services)), (
            f"{err_prefix} configuration services list cannot contain duplicates"
        )
        ...
        assert len(host_processes) == len(set(host_processes)), (
            f"{err_prefix} configuation processes list cannot contain duplicates"
        )
        ...
        for src, row in enumerate(self.topology):
            for dest, col in enumerate(row):
                if src == dest:
                    continue
                if col == 1 and (
                    str((src, dest)) not in firewall or str((dest, src)) not in firewall
                ):
                    return False
        return True
